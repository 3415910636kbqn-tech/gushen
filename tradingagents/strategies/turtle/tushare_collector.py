...
class TushareClient(
    ...
    def __init__(self, token: str = ""):
        # 数据源已替换为 akshare 桥接层（无需 tushare token）
        ...
        if api_url:
            try:
                # 桥接层对象无 _DataApi 私有属性，自定义 API URL 不生效
    ...
    def _safe_call(self, api_name: str, **kwargs) -> pd.DataFrame:
        ...
        for attempt in range(1, self.MAX_RETRIES + 1):
            try:
                api_func = getattr(self.pro, effective_name, None)
                if api_func is None:
                    # 桥接层无此接口（top10_holders/fina_mainbz/repurchase/hk_*/us_*）
                    # 返回带通用列的空表，不重试（龟龟对空表容错）
                    return pd.DataFrame(columns=["ts_code", "end_date"])
                df = api_func(**kwargs)
                # 适配：桥接层部分全市场接口忽略 ts_code 参数（如 stock_basic），
                # 按 ts_code 精确过滤一次（匹配不到则原样返回，兼容单只接口）
                filter_code = kwargs.get("ts_code")
                if filter_code and df is not None and not df.empty and "ts_code" in df.columns:
                    matched = df[df["ts_code"] == filter_code]
                    if not matched.empty:
                        df = matched.reset_index(drop=True)
                # 适配：桥接层 dividend 无 div_proc 列，龟龟 get_dividends 依赖
                # "实施" 过滤（巨潮已实施分红，补列保持语义一致）
                if effective_name == "dividend" and df is not None and not df.empty \
                        and "div_proc" not in df.columns:
                    df = df.copy()
                    df["div_proc"] = "实施"
                return df
