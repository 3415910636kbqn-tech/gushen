...
class TushareScreener:
    ...
    def _get_pro(self):
        """Lazy-initialize Tushare pro API via akshare bridge (no token)."""
    ...
    def _tier1_filter(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        #    list_date 为 NaN/None 的行跳过上市年限条件（akshare 部分股票上市
        #    日期缺失，不能因 NaN 把股票全滤掉）
        ...
        #    降级：akshare 全市场无股息率（dv_ttm 全为 None/NaN）时跳过该过滤，
        #    否则主通道会因 dv_ttm 全空被全部过滤掉（打印日志说明）
